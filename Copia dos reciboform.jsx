...
const gerarRecibo = async (dados, limparFormulario) => {
  ...
  const existingPdfBytes = await fetch("/PadraoRecibo.pdf").then((res) =>
    res.arrayBuffer()
  );
  ...
  // Nº RECIBO
  page.drawText(`N° DO RECIBO: ${safeText(numeroRecibo)}`, {
    x: 80,
    y: 735,
    size: 12,
  });
  ...
  function justifyText(text, x, y, maxWidth, lineHeight, font, page, fontSize) {
    ...
    lines.forEach((line, index) => {
      ...
      if (spacesToAdd > 0 && index < lines.length - 1) {
        ...
        justifiedText = wordsInLine.join(
          " ".repeat(Math.round(spaceBetweenWords / spaceWidth))
        );
      } else {
        justifiedText = line;
      }
  ...
  const textoRecibo = `Declaramos que recebemos a importância de R$ ${safeText(
    dados.valor
  )} (${safeText(dados.valorExtenso)}), 
da Prefeitura Municipal de ${safeText(
    dados.selectedCategory
  )}, referente a ${safeText(dados.selectedProduct)}, 
referente ao mês de: ${safeText(
    dados.mesRef
  )} para o Consórcio Intermunicipal de Saúde da 22ª Regional de Saúde de Ivaiporã.`;
  ...
  // RODAPÉ
  page.drawText(`Ivaiporã, ${formatarData(safeText(dados.data))}`, {
    x: 400,
    y: 250,
    size: 10,
    font,
  ...
  download(
    pdfBytes,
    `ReciboCIS_${dados.selectedCategory}_${dados.data}.pdf`,
    "application/pdf"
  );
...
const ReciboForm = () => {
  ...
  const handleValueChange = (e) => {
    ...
    if (inputValue) {
      ...
      const formattedValue = (parseInt(inputValue) / 100).toLocaleString(
        "pt-BR",
        {
          style: "currency",
          currency: "BRL",
  ...
  const gerarRecibo = async (dados, limparFormulario) => {
    ...
    drawCenteredText(
      page,
      `VALOR DO RECIBO: ${dados.valor} (${dados.valorExtenso})`,
      font,
      15,
      595,
      690,
        ...
        if (textWidth < maxWidth) {
          ...
          while (font.widthOfTextAtSize(word, fontSize) > maxWidth) {
            let splitIndex = Math.floor(
              (maxWidth / font.widthOfTextAtSize(word, fontSize)) * word.length
            );
    ...
    function justifyText(
      ...
      const totalTextWidth = words.reduce(
        (acc, word) => acc + font.widthOfTextAtSize(word, fontSize),
        0
      );
      const totalSpaces = words.length - 1;
      let extraSpace =
        (maxWidth - totalTextWidth - (isFirstLine ? indentSize : 0)) /
        totalSpaces;
    ...
    const textoRecibo = `Declaramos que recebemos a importância de R$ ${safeText(
      dados.valor
    )} (${dados.valorExtenso}), da Prefeitura Municipal de ${safeText(
      dados.selectedCategory
    )},referente a ${safeText(dados.selectedProduct)}, 
referente ao mês de: ${safeText(
      dados.mesRef
    ...
    wrappedText.forEach((line, index) => {
      let textToDraw =
        index < wrappedText.length - 1
          ? justifyText(line, font, 12, 450, 40, index === 0)
          : line;
